...
#define TEST_IMG_SIZE (250 * 250 * sizeof(uint16_t))
...
void lcd_panel_test(esp_lcd_panel_io_handle_t io_handle, esp_lcd_panel_handle_t panel_handle)
{
    ...
    uint16_t *def = 0;
    uint16_t color_byte = 0x1f;
    uint16_t color_byte1 = 0xF800;
    uint16_t color_byte2 = 0x07E0;

    memset(img, COLOR_WHITE, TEST_IMG_SIZE);
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, 240 ,240, img);
    vTaskDelay(3000/portTICK_PERIOD_MS);
    for(int i=0;i< TEST_IMG_SIZE;i=i+2)
    {
        def = (uint16_t *)(img+i);
        *def = color_byte;
    }
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 10, 240 ,20, img);
    vTaskDelay(3000/portTICK_PERIOD_MS);
    while(1);
    // turn off screen
